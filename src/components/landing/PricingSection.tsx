...
import { Check, Crown, Zap, Building, Loader2, AlertCircle } from 'lucide-react';
...
export function PricingSection({ onAuthRequired, isAuthenticated }: PricingSectionProps) {
  ...
  const isCurrentPlan = (plan: typeof plans[0]) => {
    ...
    if (plan.name === 'Free' && (!subscription.price_id || subscription.subscription_status !== 'active')) {
      return true;
    }
    return subscription.price_id === plan.priceId && subscription.subscription_status === 'active';
  };
