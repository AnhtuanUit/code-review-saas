	...
	const handleUpgrade = async () => {
		...
		try {
			...
			await createCheckoutSession(
				"price_1RirObHS9ff337QLCFAAHvLl",
				"subscription"
			);
		} catch (err) {
			...
			<Card className="bg-slate-800/50 border-slate-700 p-6">
				<Alert className="border-red-600/30 bg-red-600/20">
					...
					<AlertDescription className="text-red-400">
						Failed to load billing information: {String(error)}
					</AlertDescription>
				</Alert>
			</Card>
		);
						...
						<div className="space-y-4">
							<div className="bg-gradient-to-r from-cyan-600/20 to-blue-600/20 border border-cyan-600/30 rounded-lg p-4">
								...
								<p className="text-xs text-slate-300 mb-3">
									Get unlimited repositories, advanced features, and priority
									support
								</p>
								<Button
									size="sm"
									onClick={handleUpgrade}
									disabled={isCheckoutLoading || upgrading}
